classdef RL_environment < rl.env.MATLABEnvironment
    ...
    properties
        ...
        toll_contatto = 0.005;
        desired_gap = 0.05; % m - minimum longitudinal singulation gap
    ...
    methods  
        ...
        function Reward = getReward(this,AMS_actions)
            ...
            activeBoxes = 1:this.n_boxes_tot;
            forwardProgress = sum(max(0,yCurrent(activeBoxes) - yPrevious(activeBoxes)));
            ...
            separationReward = 0;
            if newExits > 0 && this.index_exit > 1
                current = this.exit_order(this.index_exit);
                previous = this.exit_order(this.index_exit-1);
                ...
                if gapError >= 0
                    separationReward = 35 + 10*tanh(8*gapError);
                else
                    separationReward = -120*abs(gapError);
                end
            elseif newExits > 0
                separationReward = 8;
            end
            ...
            Reward = 2*forwardProgress + 6*newExits + separationReward - ...
                12*crowdingPenalty - 80*overlapPenalty - 1e-3*actionEffort;
            %================= END REWARD PART ===================
        ...
        function Metrics = getSingulationMetrics(this)
            ...
            Metrics.Gaps = gaps;
            Metrics.SuccessfulPairs = sum(gaps > 0);
